...
my $sampath = "/srv/gsfs0/software/samtools/samtools-1.5/bin/samtools"; #PATH TO THE SAMTOOLS EXECUTABLE
my $sambambapath = "/srv/gsfs0/software/sambamba/0.5.9/sambamba"; #PATH TO THE SAMTOOLS EXECUTABLE
my $genomepath = "/srv/gsfs0/projects/snyder/chappell/Annotations/UCSC-hg19/WholeGenomeFasta/genome.fa"; #PATH TO REFERENCE GENOME
my $offset = 33; #BASE QUALITY SCORE OFFSET - 33 FOR SANGER SCALE, 64 FOR ILLUMINA SCALE

##END GLOBAL VARIABLES

my $bedtemp = join '', $outputfile, '.bed';
system("awk \'\$1\!\=\"chromosome\"\{print \$1\"\t\"\$2-1\"\t\"\$2\}\' $inputfile \| sort -k1,1 -k2,2n \> $bedtemp");
my $piletemp = join '', $outputfile, '.pileup';
system("source ~/.bash_profile");
#system("$sambambapath mpileup -L $bedtemp -o $piletemp -t 4 $bamfile --samtools -A -B -d 1000000 -q $minmapqual -Q $minbasequal -f $genomepath");
